...
export function buildSystemPrompt(cwd, repo) {
  ...
  // Memory graph (if initialized)
  if (getMemoryDb()) {
    const memSections = [];

    const rules = getRepoRules();
    if (rules.length > 0) {
      memSections.push("PATTERNS THAT WORK:");
      for (const r of rules.slice(0, 5)) {
        memSections.push(`- ${r.rule} (${Math.round(r.confidence * 100)}%)`);
      }
    }

    const risky = getHighRiskFiles();
    if (risky.length > 0) {
      memSections.push("CAUTION (frequently modified files):");
      for (const r of risky.slice(0, 3)) {
        memSections.push(`- ${r.path}: modified by ${r.touch_count} agents`);
      }
    }

    const failures = getRecentFailures(3);
    if (failures.length > 0) {
      memSections.push("RECENT FAILURES (avoid repeating):");
      for (const f of failures) {
        const errInfo = f.notes ? f.notes.slice(0, 80) : f.outcome;
        memSections.push(`- ${f.agent_id} on ${f.task_key}: ${errInfo}`);
      }
    }
