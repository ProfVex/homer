...
export function start(argv) {
  ...
  const ui = createUI();
  const { screen, sidebar, main, statusBar, toolSelector, issuePicker, P, c, SIDEBAR_W, panes } = ui;
  ...
  function makeAgent(id, issue, resumeCtx) {
    ...
    // Get pane dimensions for PTY
    const cols = pane.terminal.width - (pane.terminal.iwidth || 2);
    const rows = pane.terminal.height - (pane.terminal.iheight || 2);

    let proc;
    try {
      proc = spawnTool(tool, spawnOpts, Math.max(cols, 40), Math.max(rows, 10));
    } catch (e) {
      panes.remove(id);
    ...
    // Handle terminal resize for this pane
    pane.terminal.on("resize", () => {
      const w = pane.terminal.width - (pane.terminal.iwidth || 2);
      const h = pane.terminal.height - (pane.terminal.iheight || 2);
      try { proc.resize(Math.max(w, 40), Math.max(h, 10)); } catch {}
    });
  ...
  function renderSidebar() {
    ...
    if (opts.repo) {
      ...
      sidebar.repoHeader.setContent(
        `${c(P.dim, " REPO")}\n ${c(P.accent, repoParts[0])}${c(P.dimmer, "/")}${c(P.title, repoParts[1] || "")}\n${statsLine}`,
      );
    } else {
    ...
    if (state.prd) {
      ...
      for (const story of state.prd.userStories.slice(0, 10)) {
        ...
        const title = story.title.length > SIDEBAR_W - 12
          ? story.title.slice(0, SIDEBAR_W - 15) + "..."
          : story.title;
    ...
    if (issuesToShow.length > 0) {
      ...
      for (const issue of issuesToShow.slice(0, maxIssues)) {
        ...
        const title = issue.title.length > SIDEBAR_W - 10
          ? issue.title.slice(0, SIDEBAR_W - 13) + "..."
          : issue.title;
    ...
    for (let i = 0; i < state.agents.length; i++) {
      ...
      agentLines.push(`${sel} {${statusColor}-fg}${dot}{/} ${c(P.fg, a.id)} ${c(P.dim, label)} ${toolIcon}${attempts}`);
    }
  ...
  function renderStatus() {
    ...
    if (state.termFocused) {
      parts.push(`${c(P.dimmer, "\u2502")} {${P.working}-fg}\u25CF TYPING{/} ${c(P.dim, "^A=nav")}`);
    } else {
      parts.push(`${c(P.dimmer, "\u2502")} ${c(P.dim, "Enter=type ^A=toggle")}`);
    }
  ...
  function renderBar() {
    ...
    // Key hints
    const hints = [];
    if (opts.repo) hints.push(`${c(P.accent, "i")}${c(P.dim, " issue")}`);
    hints.push(`${c(P.accent, "t")}${c(P.dim, " tool")}`);
    hints.push(`${c(P.accent, "c")}${c(P.dim, " index")}`);
    hints.push(`${c(P.accent, "w")}${c(P.dim, " history")}`);
