...
// ── Write: Context Compaction ────────────────────────────────────────────────

/**
 * Record learnings extracted from agent output before context is trimmed.
 * Called by engine._extractAndStoreContext() before _trimBuffer() discards output.
 *
 * Extracts:
 *   1. File co-mentions (files discussed together → likely co-changes)
 *   2. Error patterns (mid-work errors, not just verification)
 *   3. Approach notes (what the agent was trying)
 *
 * @param {string} agentId — "agent-3"
 * @param {string|null} tKey — "story:US-001" or null
 * @param {object} data — { filePaths: string[], errors: string[], approachNote: string|null }
 */
export function recordContextCompaction(agentId, tKey, data = {}) {
  if (!db) return;

  const ts = now();
  const { filePaths = [], errors = [], approachNote = null } = data;

  // 1. Touch all mentioned files (updates touch_count)
  for (const fp of filePaths) {
    _touchFile(fp);
  }

  // 2. Record file co-mentions as co-changes if 2+ files mentioned together
  if (filePaths.length >= 2) {
    _updateCochanges(filePaths);
  }

  // 3. Create low-confidence rules from mid-work error patterns
  for (const err of errors.slice(0, 3)) {
    const scope = filePaths.length > 0 ? `file:${filePaths[0]}` : "repo";
    upsertRule(scope,
      `Mid-work error seen: "${err.slice(0, 80)}" — check for this pattern.`,
      0.3,
      `compaction:${agentId}`
    );
  }

  // 4. Store approach note as a task_run annotation
  if (tKey && approachNote) {
    const existing = db.query(
      "SELECT id, notes FROM task_runs WHERE agent_id = ? AND task_key = ? ORDER BY created_at DESC LIMIT 1"
    ).get(agentId, tKey);

    if (existing) {
      const prevNotes = existing.notes || "";
      const combined = prevNotes
        ? `${prevNotes}\n[compaction] ${approachNote.slice(0, 200)}`
        : `[compaction] ${approachNote.slice(0, 200)}`;
      db.query("UPDATE task_runs SET notes = ? WHERE id = ?").run(combined.slice(0, 800), existing.id);
    }
  }
}
...
/**
 * Get learning effectiveness metrics for the dashboard.
 * Shows whether Homer is getting smarter over time.
 *
 * @returns {object|null} — metrics object, or null if DB not open
 */
export function getLearningMetrics() {
  if (!db) return null;

  // Average verify attempts per task (trending down = improvement)
  const avgAttempts = db.query(`
    SELECT AVG(attempts) as avg, COUNT(*) as total
    FROM task_runs WHERE outcome = 'passed' AND attempts > 0
  `).get();

  // Rule effectiveness — how many rules have positive hit rate
  const ruleStats = db.query(`
    SELECT
      COUNT(*) as total,
      SUM(CASE WHEN hits > misses THEN 1 ELSE 0 END) as effective,
      AVG(confidence) as avgConfidence
    FROM repo_rules WHERE confidence > 0.1
  `).get();

  // Error resolution rate
  const solutionStats = db.query(`
    SELECT
      COUNT(*) as total,
  ...
  // Top learned rules (highest confidence, most battle-tested)
  const topRules = db.query(`
    SELECT id, scope, rule, confidence, hits, misses, updated_at
    FROM repo_rules
    WHERE confidence > 0.3 AND (hits + misses) > 0
    ORDER BY confidence DESC, hits DESC
    LIMIT 10
  `).all();

  // Recent verification episodes (last 20)
  const recentEpisodes = db.query(`
    SELECT task_key, agent_id, attempt, passed, checks, created_at
    FROM verification_episodes
    ORDER BY created_at DESC
    LIMIT 20
  `).all();

  // Error→file relations (top by occurrences)
  const topErrorFiles = db.query(`
    SELECT error_key, file_path, occurrences
    FROM error_file_relations
    ORDER BY occurrences DESC
    LIMIT 15
  `).all();

  // Learning velocity — rules created in the last 7 days
  const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
  const velocity = db.query(`
    SELECT COUNT(*) as newRules
    FROM repo_rules WHERE created_at > ?
  `).get(sevenDaysAgo);
  ...
  return {
    avgVerifyAttempts: avgAttempts?.avg ? Math.round(avgAttempts.avg * 10) / 10 : null,
    totalTasksPassed: avgAttempts?.total || 0,
    rules: {
      total: ruleStats?.total || 0,
      effective: ruleStats?.effective || 0,
      avgConfidence: ruleStats?.avgConfidence ? Math.round(ruleStats.avgConfidence * 100) : 0,
    },
    solutions: {
      total: solutionStats?.total || 0,
      resolved: solutionStats?.resolved || 0,
      resolutionRate: solutionStats?.total > 0
        ? Math.round((solutionStats.resolved / solutionStats.total) * 100)
        : 0,
      avgConfidence: solutionStats?.avgConfidence ? Math.round(solutionStats.avgConfidence * 100) : 0,
    },
    topRules,
    recentEpisodes: recentEpisodes.map(ep => ({
      ...ep,
      checks: _parseJSON(ep.checks),
    })),
    topErrorFiles,
    learningVelocity: velocity?.newRules || 0,
