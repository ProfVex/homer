...
/**
 * Truncate a string to fit within a max visible width.
 * Strips blessed tags before measuring so color codes don't inflate length.
 */
function truncate(text, maxW) {
  // Strip blessed color tags to get visible length
  const visible = text.replace(/\{[^}]*\}/g, "");
  if (visible.length <= maxW) return text;

  // We need to truncate the visible content to maxW-3 (for "...")
  // Walk through original string tracking visible chars
  let visCount = 0;
  let cutIdx = 0;
  let inTag = false;
  for (let i = 0; i < text.length; i++) {
    if (text[i] === "{" && !inTag) { inTag = true; continue; }
    if (text[i] === "}" && inTag) { inTag = false; continue; }
    if (!inTag) {
      visCount++;
      if (visCount >= maxW - 3) { cutIdx = i + 1; break; }
    }
  }
  // Close any open tags and append ellipsis
...
export function createUI() {
  ...
  // Brand header — prominent, with live status indicator
  const brandHeader = blessed.box({
    ...
    style: { bg: P.headerBg },
    padding: { left: 1 },
    content: `${c(P.magenta, "◆")} ${c(P.title, "HOMER")} ${c(P.dim, "idle")}`,
  });
  ...
  const dagBox = blessed.box({
    ...
    mouse: true,
    alwaysScroll: true,
    wrap: false,         // prevent character-level wrapping
    scrollbar: {
      ch: "▐",
  ...
  function createPane(agentId, handler) {
    ...
    // blessed.terminal uses scroll() for its internal buffer
    term.on("wheeldown", () => {
      term.scroll(3);
      screen.render();
    });
  ...
  function layoutPanes() {
    ...
    // Get actual available width in the main panel (in columns)
    const panelW = mainPanel.width || 80;
    ...
    if (n === 1) {
      ...
      // Stack vertically if panel is narrow (< 120 cols), side-by-side if wide
      if (panelW < 120) {
        // Vertical stack — each pane gets full width
        panes[0].terminal.top = 0;
        panes[0].terminal.left = 0;
        panes[0].terminal.width = "100%";
        panes[0].terminal.height = "50%";
        panes[1].terminal.top = "50%";
        panes[1].terminal.left = 0;
        panes[1].terminal.width = "100%";
        panes[1].terminal.height = "50%";
      } else {
        // Side by side — enough room for both
        panes[0].terminal.top = 0;
        panes[0].terminal.left = 0;
      ...
      if (panelW < 120) {
        ...
        const cellH = Math.floor(100 / 3);
        for (let i = 0; i < 3; i++) {
          panes[i].terminal.top = `${i * cellH}%`;
          panes[i].terminal.left = 0;
          panes[i].terminal.width = "100%";
          panes[i].terminal.height = i === 2 ? `${100 - 2 * cellH}%` : `${cellH}%`;
        }
      } else {
        panes[0].terminal.top = 0;
        panes[0].terminal.left = 0;
      ...
      // 4+ panes — use grid, but prefer fewer columns on narrow screens
      const cols = panelW >= 160 ? 3 : panelW >= 120 ? 2 : 1;
  ...
  const welcomeBox = blessed.box({
    ...
    content: [
      ...
      `  ${c(P.accent, "j")}      ${c(P.fg, "join agent")}       ${c(P.accent, "^G")}   ${c(P.fg, "join (any mode)")}`,
      `  ${c(P.accent, "^A")}     ${c(P.fg, "toggle focus")}     ${c(P.accent, "^N")}   ${c(P.fg, "new agent (any mode)")}`,
      `  ${c(P.accent, "q")}      ${c(P.fg, "quit")}`,
      "",
      `  ${c(P.dimmer, "─".repeat(44))}`,
  ...
  return {
    screen,
    sidebar: { brandHeader, toolBadge, repoHeader, dagBox, agentList },
