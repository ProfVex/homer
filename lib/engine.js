...
export class HomerEngine extends EventEmitter {
  constructor(opts = {}) {
    ...
    // MCP config path (resolved in init)
  ...
  async init() {
    ...
    // MCP config discovery: project-local first, then global
    const mcpLocal = resolve(process.cwd(), ".homer", "mcp.json");
    const mcpGlobal = join(homedir(), ".homer", "mcp.json");
    if (existsSync(mcpLocal)) this.mcpConfigPath = mcpLocal;
    else if (existsSync(mcpGlobal)) this.mcpConfigPath = mcpGlobal;
  ...
  _makeAgent(id, issue, resumeCtx = null) {
    ...
    if (tool.supportsMcp && this.mcpConfigPath) {
      spawnOpts.mcpConfigPath = this.mcpConfigPath;
    }
